...
class TubeVisual(MeshVisual):
    ...
    def generate_new_vertices(self, points):
        ...
        for i in range(len(points)):
            ...
            v = (
                np.arange(tube_points, dtype=np.float32)
                / tube_points
                * 2
                * np.pi
            )
            cx = (
                -1.0 * radius * np.cos(v)
            )  # *(1 + np.random.random(tube_points)*0.1)
            cy = radius * np.sin(v)  # *(1 + np.random.random(tube_points)*0.1)
            grid[i] = (
                pos + cx[:, np.newaxis] * normal + cy[:, np.newaxis] * binormal
            )
...
def _frenet_frames(points, closed):
    ...
    for i in range(1, len(points)):
        ...
        if norm(vec) > epsilon:
            ...
            normals[i] = rotate(-np.degrees(theta), vec)[:3, :3].dot(
                normals[i]
            )
    ...
    if closed:
        ...
        for i in range(1, len(points)):
            normals[i] = rotate(-np.degrees(theta * i), tangents[i])[
                :3, :3
            ].dot(normals[i])
...
class TubeVisualizer:
    def __init__(self):
        ...
        self.canvas = scene.SceneCanvas(
            keys="interactive", bgcolor="white", show=True
        )
        ...
        # Create a tube
        self.tube_points = np.ones((n_points, 3)) * np.arange(
            n_points
        ).reshape(-1, 1)
        ...
        self.tube = Tube(
            radius=1, shading="smooth", tube_points=10, parent=self.view.scene
        )
